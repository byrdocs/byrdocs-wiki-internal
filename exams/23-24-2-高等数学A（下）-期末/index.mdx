...
3. 已知级数 $\sum_{n=1}^{\infty}\frac{(-1)^{n} }{n^{p} }$ 条件收敛, 则常数 $p$ 的取值范围是 <Blank>$p\ge1$</Blank>.
...
7. 设 $\Omega$ 是球体闭区域 $\left\{(x,y,z)\mid x^{2}+y^{2}+z^{2}\le1\right\}$, 则 $\iiint_{\Omega}x^{2}\mathrm{d}x\,\mathrm{d}y\,\mathrm{d}z=$ <Blank>$\frac{4}{15}$</Blank>.
